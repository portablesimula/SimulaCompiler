...
public class DirectFile$ extends ImageFile$ {
	...
	public void locate(final int p) {
		if (p < 1 | p > MAXLOC$)
			throw new SimulaRuntimeError("Locate: Parameter out of range");
		else
			try {
				randomAccessFile.seek((p - 1) * RECORDSIZE$);
			} catch (IOException e) {
				throw new SimulaRuntimeError("Locate failed", e);
			}
		LOC$ = p;
	}
	...
	public int lastloc() {
		...
		try {
			...
			throw new SimulaRuntimeError("Lastloc failed", e);
		}
	}
	...
	public void inimage() {
		...
		if (!OPEN$)	throw new SimulaRuntimeError("File not opened");
		if (TXT$.length(image) != RECORDSIZE$)
			throw new SimulaRuntimeError("DirectFile image length changed");
		...
		// else if(external image does not exists) fill=(char)0;
			...
			try {
				...
				throw new SimulaRuntimeError("Inimage failed", e);
		}
		ENDFILE$= LOC$ > lastloc();
		locate(LOC$ + 1);
	...
	public void outimage() {
		...
		if (LOC$ > MAXLOC$)
			throw new SimulaRuntimeError("file overflow");
		try {
			randomAccessFile.write(image.edText().getBytes());
		} catch (IOException e) {
			throw new SimulaRuntimeError("Outimage failed", e);
		}
		locate(LOC$ + 1);
		ASGTXT$(image,null);
		setpos(1);
