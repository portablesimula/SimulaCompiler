...
public final class TXT$ {
	...
	public static char getchar(TXT$ T) {
		if (T.POS >= T.LENGTH) {
			throw new SimulaRuntimeError("Getchar: pos >= length  pos="+T.POS+", length="+T.LENGTH);
		} else {
			T.POS++;
	...
	private static void checkAssignable(TXT$ T) {
		if (T==null || T.OBJ == null) throw new SimulaRuntimeError("Illegal Text T.put...  T==NOTEXT");
		if(T.OBJ.CONST) {
			
			throw new SimulaRuntimeError("Illegal Text T.put...  T is constant, T="+T);
		}
	}

	public static void putchar(final TXT$ T,final char c) {
		checkAssignable(T);
		if (T.POS >= T.LENGTH) throw new SimulaRuntimeError("Putchar outside frame");
...
//		if (i < 0 || n < 0 || i + n > LENGTH + 1)  // NOTE: FEIL I SIMULA STANDARD
		if (i <= 0)	throw new SimulaRuntimeError(" ! Sub(i="+i+",n="+n+") is out of frame, i <= 0");
		if (n < 0)throw new SimulaRuntimeError(" ! Sub(i="+i+",n="+n+") is out of frame, n < 0");
		if (T==null) T=ENVIRONMENT$.NOTEXT;
		if (i + n > T.LENGTH + 1)
			throw new SimulaRuntimeError(" ! Sub(i="+i+",n="+n+") is out of frame, i + n > LENGTH + 1");
		else if (n > 0) {
			TXT$ U = new TXT$();
			U.OBJ = T.OBJ;
			U.START = T.START + i - 1;
...
//		if(T==null) T=ENVIRONMENT$.NOTEXT;
	...
	public static void putfix(final TXT$ T,double r, int n) {
		// TODO: Complete the implementation according to Simula Standard Definition.
		if(n<0) throw new SimulaRuntimeError("putfix(r,n) - n < 0");
	...
	public static void putreal(final TXT$ T,double r,int n) {
		// TODO: Complete the implementation according to Simula Standard Definition.
		if(n<0) throw new SimulaRuntimeError("putreal(r,n) - n < 0");
