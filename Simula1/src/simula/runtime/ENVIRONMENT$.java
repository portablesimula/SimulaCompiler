...
import simula.runtime.RT.Option;
...
public class ENVIRONMENT$ extends RTObject$ {
	...
	// ************************************************************
	public static boolean IS$(final Object obj,final Class<?> cls) {
	...
	public static int IPOW$(final int b,int x) {
		...
		if (x == 0) {
			if (b == 0)
				throw new SimulaRuntimeError("Exponentiation: " + b + " ** " + x + "  Result is undefined.");
			return (1); // any ** 0 ==> 1
		} else if (x < 0)
			throw new SimulaRuntimeError("Exponentiation: " + b + " ** " + x + "  Result is undefined.");
		else if (b == 0)
	...
	public static int IPOW$EXACT(final int b,int x) {
...
//			v = v * b;
	...
	public static char lowten(final char c) {
		if (illegalLowten(c))
			throw new SimulaRuntimeError("Illegal LOWTEN Character: " + c + "  Code=" + (int) c);
	...
	public static char decimalmark(final char c) {
		char decimalmark = 0;
		if (c != '.' && c != ',') {
			throw new SimulaRuntimeError("Decimalmark error: "+c);
		} else {
			decimalmark = CURRENTDECIMALMARK;
			CURRENTDECIMALMARK = c;
		}
	 ...
	 * @return
	 */
	public static TXT$ upcase(TXT$ t) {
		if (t == null) t = NOTEXT;
		String s=t.edText().toUpperCase();
		return(ASGTXT$(t,new TXT$(s)));
//		return(new TXT$(t.edText().toUpperCase()));
	}
	 ...
	 * @return
	 */
	public static TXT$ lowcase(TXT$ t) {
		if (t == null) t = NOTEXT;
		String s=t.edText().toLowerCase();
		return(ASGTXT$(t,new TXT$(s)));
//		return(new TXT$(t.edText().toLowerCase()));  blanks
	}
	...
	public static TXT$ max(final TXT$ x,final TXT$ y) {
	...
	public static TXT$ min(final TXT$ x,final TXT$ y) {
	 ...
	 * @param msg
	 */
	public static void error(final TXT$ msg) {
		throw new SimulaRuntimeError(msg.edText());
...
//		terminate_program();
	 ...
	 * @return
	 */
	public static int lowerbound(final ARRAY$<?> a,final int i) {
		try {
			return (a.LB[i - 1]);
		} catch (SimulaRuntimeError e) {
			throw new SimulaRuntimeError("Wrong number of dimensions", e);
		}
	}
	 ...
	 * @return
	 */
	public static int upperbound(final ARRAY$<?> a,final int i) {
		try {
			return (a.UB[i - 1]);
		} catch (SimulaRuntimeError e) {
			throw new SimulaRuntimeError("Wrong number of dimensions", e);
...
//	}
	 ...
	 * @return
	 */
//	public static boolean draw(float a, NAME$<Integer> U) {
...
//		else val = a >= basicDRAW(U);
	 ...
	 * @return
	 */
//	public static int randint(int a, int b, NAME$<Integer> U) {
	public static int randint(final int a,final int b,final int U) {
		if (b < a) throw new SimulaRuntimeError("Randint(a,b,u):  b < a");
//		int val = entier(basicDRAW(U) * ((b - a + 1))) + a;
	 ...
	 * @return
	 */
//	public static double uniform(double a, double b, NAME$<Integer> U) {
	public static double uniform(final double a,final double b,final int U) {
		if (b < a) throw new SimulaRuntimeError("Uniform(a,b,u): b < a");
//		double val = a + ((b - a) * basicDRAW(U));
	 ...
	 * @return
	 */
//	public static double normal(double a, double b, NAME$<Integer> U) {
	public static double normal(final double a,final double b,final int U) {
		double t, p, q, v, x;
		boolean z;
		if (b < 0.0) throw new SimulaRuntimeError("Normal(a,b,u):  b <= 0.");
//		v = basicDRAW();
		v = random.nextDouble();
		if (v > 0.5) {
			z = true;
	 ...
	 * @return
	 */
//	public static double negexp(double a, NAME$<Integer> U) {
	public static double negexp(final double a,final int U) {
		if (a <= 0.0) throw new SimulaRuntimeError("Negexp(a,u): a <= 0");
//		double v = basicDRAW(U);
	 ...
	 * @return
	 */
//	public static int Poisson(double a, NAME$<Integer> U) {
...
//				acc = acc * basicDRAW();
	 ...
	 * @return
	 */
//	public static double Erlang(double a, double b, NAME$<Integer> U) {
	public static double Erlang(final double a,final double b,final int U) {
		int c;
		double val, bc, ab, z, v;
		if (a <= 0.0 || b <= 0.0)
			throw new SimulaRuntimeError("Erlang(a,b,u):  a <= 0  or  b <= 0");
		val = 0;
		c = entier(b);
		bc = b - c;
		ab = a * b;
...
//			v = basicDRAW(U);
	 ...
	 * @return
	 */
//	public static int discrete(ARRAY$<double[]> A, NAME$<Integer> U) {
	public static int discrete(final ARRAY$<double[]> A,final int U) {
		int result, j, nelt;
		double v;
		int lb = A.LB[0];
		int ub = A.UB[0];
//		v = basicDRAW(U);
	 ...
	 * @return
	 */
//	public static double linear(ARRAY$<double[]> A, ARRAY$<double[]> B, NAME$<Integer> U) {
	public static double linear(final ARRAY$<double[]> A,final ARRAY$<double[]> B,final int U) {
		int i, nelt;
		...
		if (nelt != (B.UB[0] - B.LB[0] + 1))
			throw new SimulaRuntimeError("Linear(A,B,U): The number of elements in A and B are different.");
//		v = basicDRAW(U);
		v = random.nextDouble();
		i = 0;
		while (A.Elt[i] < v)
		...
		if (i == 0) {
			if (v == 0.0 && A.Elt[i] == 0.0)
				...
				throw new SimulaRuntimeError("Linear(A,B,U): The array a does not satisfy the stated assumptions.");
		} else if (i >= nelt)
			throw new SimulaRuntimeError("Linear(A,B,U): The array a does not satisfy the stated assumptions.");
	 ...
	 * @return
	 */
//	public static int histd(ARRAY$<float[]> A, NAME$<Integer> U) {
	public static int histd(final ARRAY$<float[]> A,final int U) {
		int result = 0;
		int j; // Array index.
		...
		do {
			tmp = A.Elt[j];
			if (tmp < 0.0)
				throw new SimulaRuntimeError("Histd(a,u):  An element of the array a is negative");
			sum = sum + tmp;
			j = j + 1;
		} while (j < nelt);
//		wsum = sum * basicDRAW(U); // Make 0 <= wsum < sum
	...
	public static double clocktime() {
		...
		// RT.println("ClockTime: Hour="+hour+", Minute="+minute+", Second="+second);
	 ...
	 * @param d
	 */
	public static void histo(final ARRAY$<?> A,final ARRAY$<?> B,final float c,final float d) {
		if(A.nDim()!=1) 
			throw new SimulaRuntimeError("histo(A,B,c,d) - A is not one-dimensional");
		if(B.nDim()!=1) 
			throw new SimulaRuntimeError("histo(A,B,c,d) - B is not one-dimensional");
		int nelt = B.UB[0] - B.LB[0] + 1;
		if (nelt >= (A.UB[0] - A.LB[0] + 1))
			throw new SimulaRuntimeError("histo(A,B,c,d) - A'length <= B'length");
		try {
			...
			throw new SimulaRuntimeError("histo(A,B,c,d) - Internal Error",e);
		}
	}
	...
	// **********************************************************************
	public static TXT$ edit(final boolean n) { return(new TXT$(""+n)); }
	public static TXT$ edit(final char n)    { return(new TXT$(""+n)); }
	public static TXT$ edit(final long n)    { return(new TXT$((""+n))); }
	public static TXT$ edit(final float n)   { return(new TXT$((""+n).replace('E','&'))); }
	public static TXT$ edit(final double n)  { return(new TXT$((""+n).replace("E","&&"))); }
	public static TXT$ edfix(final double r,final int n) {
		TXT$ T=blanks(n+10); TXT$.putfix(T, r, n);
		String S=T.edText().trim();
		return(new TXT$(S));
	}
	public static TXT$ edtime(double hours) {
		return(new TXT$(LocalTime.ofSecondOfDay((long) (hours*60*60)).toString()));
	}
	...
	public static void printThreadList(final boolean withStackTrace) {
		ThreadUtils.printThreadList(withStackTrace);
	}
    ...
    public static void DEFEXCEPTION(final PRCQNT$ EXCEPTION_HANDLER) {
    ...
    public static int hash(final TXT$ txt) {
    ...
    public static char loadChar(final TXT$ t,final int i) {
    ...
    public static void storeChar(final char c,final TXT$ t,final int i) {
     ...
     *
     */
    public static TXT$ getTextInfo(final int index) {
    	//RT.println("getTextInfo: index="+index);
    	switch(index) {
    	case 1:
    		String sourceFileName=RT.SPORT_Option.getSourceFileName();
    		if(Option.VERBOSE) RT.println("getTextInfo(1)  SourceFileName="+sourceFileName);
    		return(new TXT$(sourceFileName));
    	case 2:
    		String listingFileName=RT.SPORT_Option.ListingFileName;
    		if(Option.VERBOSE) RT.println("getTextInfo(2)  Listing File Name="+listingFileName);
    		if(listingFileName==null) return(null);  // no Listing
    		return(new TXT$(listingFileName));       // listname  i.e.  Listing File Name
    	case 4:
    		String sCodeFileName= RT.SPORT_Option.getSCodeFileName();
    		if(Option.VERBOSE) RT.println("getTextInfo(4)  S-Code File Name="+sCodeFileName);
    		return(new TXT$(sCodeFileName));
    	case 7:
    		String scratchFileName= RT.SPORT_Option.getScratchFileName();
    		if(Option.VERBOSE) RT.println("getTextInfo(7)  ScratchFileName="+scratchFileName);
    		return(new TXT$(scratchFileName));  // FileName for intermediate code
    	case 11:
    		String attributeOutputFileName= RT.SPORT_Option.getAttributeOutputFileName();
    		if(Option.VERBOSE) RT.println("getTextInfo(11) AttributeOutputFileName="+attributeOutputFileName);
    		return(new TXT$(attributeOutputFileName));  // AttributeInputFileName
    	case 12:
    		String attributeFileName= RT.SPORT_Option.getAttributeFileName();
    		if(Option.VERBOSE) RT.println("getTextInfo(12) AttributeFileName="+attributeFileName);
    		return(new TXT$(attributeFileName));
    	case 13: // What is the environment part of the program head? 
    		if(Option.VERBOSE) RT.println("getTextInfo(13) environment part of the program head="+"ENVIRHEAD");
    		return(new TXT$("ENVIRHEAD"));
    	case 14: // Module identifier to be used for the current separate compilation?
    		if(Option.VERBOSE) RT.println("getTextInfo(14) Module identifier: "+RT.SPORT_Option.xDecl1);
    		return(new TXT$(RT.SPORT_Option.xDecl1));
    	case 15: // Module check code to be used for the current separate compilation?
    		String checkCode="abracadab";
    		if(Option.VERBOSE) RT.println("getTextInfo(15) Module check code: "+checkCode);
    		return(new TXT$(checkCode));
    	case 16:
    		if(Option.VERBOSE) RT.println("getTextInfo(16) Selectors="+RT.SPORT_Option.Selectors);
    		return(new TXT$(RT.SPORT_Option.Selectors)); // options and selectors
    	case 19:
    		if(Option.VERBOSE) RT.println("getTextInfo(19) PredefFileName="+RT.SPORT_Option.PredefFileName);
    		return(new TXT$(RT.SPORT_Option.PredefFileName)); // Name of attribute file for the predefined classes and procedures (the class PREDEF)?
    	case 22:
    		if(Option.VERBOSE) RT.println("getTextInfo(22) XmessageFileName="+RT.SPORT_Option.XmessageFileName);
    		return(new TXT$(RT.SPORT_Option.XmessageFileName)); // Name of a file containing seldom used information for the front end compiler, such as extended error messages.
    	case 25:
    		File file=new File(RT.SPORT_Option.getSourceFileName());
    		String ModuleIdent=file.getName();
    		//RT.println("ModuleIdent="+ModuleIdent+" ==> "+ModuleIdent.substring(0,ModuleIdent.length()-4));
    		ModuleIdent=ModuleIdent.substring(0,ModuleIdent.length()-4).toLowerCase();
    		if(Option.VERBOSE) RT.println("getTextInfo(25) ModuleIdent=\""+ModuleIdent+'"');
    		return(new TXT$(ModuleIdent)); // Modid par of source file name in lower case 
    	default: RT.NOT_IMPLEMENTED("getTextInfo: "+index); // TODO: Implement it
    	}
    	return(null);
    }
...
//    static int SimobLevel=0;
    public static int getIntInfo(final int index) {
    	//RT.println("getIntInfo: index="+index);
    	switch(index) {
    		case 1:
        		if(Option.VERBOSE) RT.println("getIntInfo(1)   GenerateScode="+RT.SPORT_Option.GenerateScode);
    			return(RT.SPORT_Option.GenerateScode);
    		case 4:
        		if(Option.VERBOSE) RT.println("getIntInfo(4)   MaxErrors="+RT.SPORT_Option.MaxErrors);
    			return(RT.SPORT_Option.MaxErrors);  // maxerrno
    		case 5:
        		if(Option.VERBOSE) RT.println("getIntInfo(5)   GiveNotes="+RT.SPORT_Option.GiveNotes);
    			return(RT.SPORT_Option.GiveNotes);  // GiveNotes
    		case 7:
        		if(Option.VERBOSE) RT.println("getIntInfo(7)   ListingImageLength=260");
    		...
    		case 10: // ARRAY BOUNDS CHECK ?  Result: 0 - Complete checking, 1 - Partial checking, 2 - No checking will be done.
    			return(0);
    		case 16:
        		if(Option.VERBOSE) RT.println("getIntInfo(16)   TraceLevel="+RT.SPORT_Option.TraceLevel);
    			return(RT.SPORT_Option.TraceLevel);  // Image length for the listing file
    		case 22:
        		if(Option.VERBOSE) RT.println("getIntInfo(22)  Recompilation="+RT.SPORT_Option.Recompilation);
    			return(RT.SPORT_Option.Recompilation);  // recomp
    		case 30:
        		if(Option.VERBOSE) RT.println("getIntInfo(30)  SimobLevel="+RT.SPORT_Option.SimobLevel);
    			return(RT.SPORT_Option.SimobLevel);  // simob_level
    		default: RT.NOT_IMPLEMENTED("getIntInfo: "+index); // TODO: Implement it
    	}
    	return(0);
    }
     ...
     * 
     */
    public static void giveTextInfo(final int index,final TXT$ info) {
    	if(Option.VERBOSE) RT.println("giveTextInfo: index="+index+", info="+info.edText());
    	switch(index) {
    	case 1: RT.SPORT_Option.xDecl1=info.edText(); // got predefmodule when compiling PREDEF.DEF
    	case 2: RT.SPORT_Option.moduleIdent=info.edText();
    	        //System.out.println("RT.SPORT_Option.moduleIdent="+RT.SPORT_Option.moduleIdent);
    	        break;
//    	case 3: RT.SPORT_Option.xDecl2=info.edText(); break;
    	case 4: RT.SPORT_Option.SPORT_SysInsertDirName=info.edText(); break;
    	default: RT.NOT_IMPLEMENTED("giveTextInfo");
    	}
    }
    ...
    public static void rts_utility(final int index,final int level) {
    	if(Option.VERBOSE) RT.println("rts_utility: index="+index+", level="+level);
		printStaticChain();
    	...
    	case 1: return;
    	default: 
    		RT.NOT_IMPLEMENTED("rts_utility: index="+index+", level="+level); // TODO: Implement it
    		printStaticChain();
    		Thread.dumpStack();
    	}
    }
