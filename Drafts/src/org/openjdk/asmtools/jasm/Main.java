...
public class Main {
    ...
    private synchronized boolean parseArgs(String argv[]) {
        ...
        for (int i = 0; i < argv.length; i++) {
            String arg = argv[i];
        	System.out.println("org.openjdk.asmtools.jasm.Main.parseArgs: arg="+arg); // MYH
            switch (arg) {
                case "-v":
                    traceFlag = true;
                    break;
                ...
                case "-d":
                	...
                	System.out.println("org.openjdk.asmtools.jasm.Main.parseArgs: destDir="+destDir); // MYH
                    if (!destDir.exists()) {
    ...
    public synchronized boolean compile(String argv[]) {
    	System.out.println("org.openjdk.asmtools.jasm.Main.compile: BEGIN"); // MYH
        // Reset the state of all objs
        reset();

        boolean validArgs = parseArgs(argv);
        if (!validArgs) {
        	System.out.println("org.openjdk.asmtools.jasm.Main.compile: !validArgs"); // MYH
            return false;
        }
