...
{ 
  ...
  private static StringBuilder accum;
  private static boolean skipComments;
  ...
  */
  public PreScanner(Reader theReader) {
...
//	  current=PreProcessor.getNext();
	  current=readNextCharacter();
	  if(accum==null) accum=new StringBuilder();
	  if(current!='\r') accum.append((char)current);
	  return(current);
  }
  ...
  private static void pushBack(int chr) {
	  accum.deleteCharAt(accum.length()-1);  // TODO: eller for ... 
	  
	  //PreProcessor.pushBack(chr);
  ...
  { String text=(accum==null)?"":accum.toString();
  ...
  private Token scanToken() {
	  ...
	  if (skipComments)
		...
		while (token!=null && token.getKeyWord() == KeyWord.COMMENT);
		  ...
		  if(firstToken) {
			  token=newItem(KeyWord.NEWLINE,1); firstToken=false; }
		  else token = scanBasic();    
	  }
	  return(token);
  }
...
//	    case PreProcessor.EOF:
	    ...
	    case '\n':			/* NL (LF) */
	      ...
	      if (!skipComments) return(newItem(KeyWord.NEWLINE,lineNumber));
	    case ' ':
	    case '\b':			/* BS */
	    case '\t':			/* HT */
	    //case '\v':		/* VT */
...
//	           checkFullLineComment();
  ...
  { while(getNext()!='\n'); //"First character of line";
    //Util.BREAK("PreProcessor.scanDirectiveLine: char="+(char)currentCharacter);
  	String directive=accum.toString().trim();
    pushBack('\n');
    doTreatDirective(directive);
  ...
  private void doTreatDirective(String directive)
  {
	...
	if(tokenizer.hasMoreTokens()) {
		...
		if(id.equalsIgnoreCase("%")); // Comment 
