 ...
 */
package simula.compiler.engineering.bcel;
...
/**
 * Based on the Apache Commons 'BCEL' Byte Code Engineering Library.
 * Main page: https://commons.apache.org/proper/commons-bcel/
 * 
...
//            javaClass.setMajor(49);
            System.out.println("*** ByteCodeEngineering.doRepairSingleByteCode: MajorVersion="+javaClass.getMajor());
            //Util.BREAK("*** ByteCodeEngineering.doRepairSingleByteCode: MajorVersion="+javaClass.getMajor());
            javaClass.setMajor(50);
	...
	private Method treatSTM_Method(MethodGen methodGen,ConstantPool constantPool) {
        ...
        if(TRACE_REPAIRING) System.out.println("*** PASS 2 - FIND AND TREAT JUMPTABLE$ ***");
        for(InstructionHandle handle:instructionList) {
        	Instruction ins=handle.getInstruction();
            if(ins instanceof INVOKESTATIC) {
            	String methodName=getMethodName((INVOKESTATIC)ins, constantPool);
        ...
        { System.out.println("*** LIST REPAIRED INSTRUCTION LIST ***");
          for(InstructionHandle handle:instructionList) {
