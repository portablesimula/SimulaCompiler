...
public final class _TXT {
	...
	public String toString() {
		return ("_TEXT: START=" + START + ", LENGTH=" + LENGTH + ", POS="
				+ POS + ", OBJ=" + OBJ);
	}
	...
	private static void checkAssignable(_TXT T) {
		if (T==null || T.OBJ == null) throw new _SimulaRuntimeError("Illegal Text T.put...  T==NOTEXT");
		if(T.OBJ.CONST) {
			
			throw new _SimulaRuntimeError("Illegal Text T.put...  T is constant, T="+T);
		}
	}

	public static void putchar(final _TXT T,final char c) {
		checkAssignable(T);
		if (T.POS >= T.LENGTH) throw new _SimulaRuntimeError("Putchar outside frame");
...
//		if (i < 0 || n < 0 || i + n > LENGTH + 1)  // NOTE: FEIL I SIMULA STANDARD
		if (i <= 0)	throw new _SimulaRuntimeError(" ! Sub(i="+i+",n="+n+") is out of frame, i <= 0");
	...
	private static String getRealItem(final _TXT T) {
		...
		while (T.POS < T.LENGTH) { // KEEP DIGITS
			...
			else if(c==_ENVIRONMENT.CURRENTDECIMALMARK) c='.'; // OK
			else if(c=='+'); // OK
			else if(c=='-'); // OK
			else if(c==_ENVIRONMENT.CURRENTLOWTEN) c='E'; // OK
//			else { T.POS=lastDigPos-1; break; }	
			else { T.POS=lastDigPos; break; }	// ØM  18/8-2021
	...
	private static String getFracItem(final _TXT T) {
		...
		while (T.POS < T.LENGTH) { // KEEP DIGITS
			...
			else if(c==_ENVIRONMENT.CURRENTDECIMALMARK); // OK  NOTE: THIS WAS WRONG IN PC-SIMULA
			else if(c==' '); // OK
			else break;
			T.POS++;
		}
    ...
    private static void putRealResult(final _TXT T,String output)
    { if(_ENVIRONMENT.CURRENTDECIMALMARK!=',')
    	 output=output.replace(',',_ENVIRONMENT.CURRENTDECIMALMARK);
      putResult(T,output);
    }
	 ...
	 * @param s
	 */
	private static void putResult(_TXT T,final String s) {
//		if(T==null) T=_ENVIRONMENT.NOTEXT;
		checkAssignable(T);
		char[] c = s.toCharArray();
		if (c.length > T.LENGTH) {
		    _RT.numberOfEditOverflows++;
			...
			for (int j = T.LENGTH-1; j >= 0; j = j - 1) {
				char k = (m>=0) ? c[m--] : ' ';
				if(Character.toUpperCase(k)=='E') k=_ENVIRONMENT.CURRENTLOWTEN;
	...
	public static void putfrac(final _TXT T,final int val,final int n) {
	    ...
	    try { while( (v>0) || (d<n) ) {
	              ...
	              item[p--]=(char)(c+'0');
	              r=r-1; d=d+1;
	              if(d==n) item[p--]=_ENVIRONMENT.CURRENTDECIMALMARK;
		      } 
	          if(val<0) item[p--]='-';
	          while(p>=0) item[p--]=' ';
