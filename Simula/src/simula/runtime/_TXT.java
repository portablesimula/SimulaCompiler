...
public final class _TXT {
	...
	private static void checkAssignable(_TXT T) {
		if (T == null || T.OBJ == null)
			throw new _SimulaRuntimeError("Illegal Text T.put...  T==NOTEXT");
		if (T.OBJ.CONST) {
	...
	public static void putchar(final _TXT T, final char c) {
		checkAssignable(T);
		if (T.POS >= T.LENGTH)
			throw new _SimulaRuntimeError("Putchar outside frame");
...
//		if (i < 0 || n < 0 || i + n > LENGTH + 1)  // NOTE: FEIL I SIMULA STANDARD
		if (i <= 0)
			throw new _SimulaRuntimeError(" ! Sub(i=" + i + ",n=" + n + ") is out of frame, i <= 0");
		if (n < 0)
			throw new _SimulaRuntimeError(" ! Sub(i=" + i + ",n=" + n + ") is out of frame, n < 0");
		if (T == null)
			T = _ENVIRONMENT.NOTEXT;
		if (i + n > T.LENGTH + 1)
	...
	private static String getRealItem(final _TXT T) {
		...
		while (T.POS < T.LENGTH) { // KEEP DIGITS
			...
			if (Character.isDigit(c))
				; // OK
			else if (c == _ENVIRONMENT.CURRENTDECIMALMARK)
				c = '.'; // OK
			else if (c == '+')
				; // OK
			else if (c == '-')
				; // OK
			else if (c == _ENVIRONMENT.CURRENTLOWTEN)
				c = 'E'; // OK
			else {
				T.POS = lastDigPos;
				break;
	...
	public static void putfix(final _TXT T, double r, int n) {
		if (n < 0)
			throw new _SimulaRuntimeError("putfix(r,n) - n < 0");
		if (n == 0) {
			putint(T, (int) (r + 0.5));
			return;
		}
	...
	public static void putfrac(final _TXT T, final int val, final int n) {
		...
		try {
			while ((v > 0) || (d < n)) {
				...
				d = d + 1;
				if (d == n)
					item[p--] = _ENVIRONMENT.CURRENTDECIMALMARK;
			}
			if (val < 0)
				item[p--] = '-';
			while (p >= 0)
