...
public class _DEC_Lib extends _CLASS {
	...
	public static int checkint(_NAME<_TXT> t) {
		NumberChecker<Number> intChecker = DECLIB.new NumberChecker<Number>() {
			public Integer check(_TXT T){ return(_TXT.getint(T)); }};
	...
	public static int checkfrac(_NAME<_TXT> t) {
		NumberChecker<Number> fracChecker = DECLIB.new NumberChecker<Number>() {
			public Integer check(_TXT T){ return(_TXT.getfrac(T)); }};
	...
	public static int checkreal(_NAME<_TXT> t) {
		NumberChecker<Number> realChecker = DECLIB.new NumberChecker<Number>() {
			public Double check(_TXT T){ return(_TXT.getreal(T)); }};
...
//		if(SIMOB.isPresent) _RT.NOT_IMPLEMENTED("DECLIB: enterdebug");
	...
	public static void exit(int code) {
		switch(code) {
			...
			default: _RT.println("Parameter to exit out of range (0,2)");
					 terminate_program();
		}
	}
	...
	public static char fetchar(_TXT t,int p) {
		if(p > 0 & p <= t.LENGTH) {
			t.POS=p-1; return(_TXT.getchar(t));
		}
		return(0);	
	}
	...
	public static _TXT from(_TXT t,int p) {
		if(p<t.LENGTH) {
			if(p<0) return(t);
			else return(_TXT.sub(t, p, t.LENGTH-p+1));
		}
		return(null);
	}
	...
	public static _TXT front(_TXT t) {
		if(t!=null) return(_TXT.sub(t,1,t.POS));
		return(null);	
	}
	...
	public static char insinglechar() {
		try { return(_RT.console.read());
		} catch(Exception e) {
			throw new _SimulaRuntimeError("Procedure insinglechar is undefined: re-run program with Runtime Option USE_CONSOLE=true");
		}
	}
	...
	public static char scanchar(_NAME<_TXT> t) {
		_TXT T=t.get();
		if(!_TXT.more(T)) return(0);
		char res=_TXT.getchar(T); t.put(T);
		return(res);		
	}
	...
	public static double scanreal(_NAME<_TXT> t) {
		...
		if(T!=null) {
			_TXT TAIL=rest(T);
			try { double res=_TXT.getreal(TAIL);
				  T.POS=T.POS+TAIL.POS; t.put(T);
				  return(res);
			} catch (Exception e) { }
		}
	...
	public static _TXT scanto(_NAME<_TXT> t,char c) {
		...
		if(index >= 0) { // Character found at index in tail
			...
			if(VERBOSE) System.out.println("scanto(1): tail="+tail+", length="+_TXT.length(TXT)+", index="+index+", pos="+pos);
//			return(_TXT.sub(TXT,TXT.POS+1,tail.length()-index));
			...
			if(VERBOSE) System.out.println("scanto(2): tail="+tail+", length="+_TXT.length(TXT)+", index="+index+", pos="+pos);
	...
	public static _TXT tsub(_TXT t,int pos,int length) {
		try { return(_TXT.sub(t,pos,length));
	...
	public static boolean upcompare(_TXT master,_TXT test) {
		...
		if(VERBOSE) System.out.println("upcompare pos="+_TXT.pos(master));
