...
public class imagefile$ extends FILE$ {
	...
	public void outimage() { // Needs redefinition
		throw new SimulaRuntimeError("Internal Error");
	}
	...
	public TXT$ FIELD$(final int w) {
		if (w > length())
			throw new SimulaRuntimeError("Item too long in output operation");
		if (pos() + w - 1 > length())
			outimage();
...
//		if (!more()) inimage();  // From Infile
	...
	private TXT$ nextItem() {
		if (lastitem())	throw new SimulaRuntimeError("Attempt to read past EOF");
