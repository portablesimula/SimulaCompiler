...
public class _Drawing extends _Simset {
    ...
    public _Head renderingSet() { return(RENDERING_SET); }
    ...
    public void setFontSize(final float size) {
    	if(DEBUG) _RT.println("SetFontSize: size="+size+", OldFont="+currentFont);
	...
	{ _ShapeElement elt=new _ShapeElement(this)._STM(); elt.drawLine(x1,y1,x2,y2); return(elt); }
	   
	public _ShapeElement drawEllipse(final double x,final double y,final double width,final double height)
	{ _ShapeElement elt=new _ShapeElement(this)._STM(); elt.drawEllipse(x,y,width,height); return(elt); }

	public _ShapeElement drawRectangle(final double x,final double y,final double width,final double height)
	{ _ShapeElement elt=new _ShapeElement(this)._STM(); elt.drawRectangle(x,y,width,height); return(elt); }
	   
	public _ShapeElement drawRoundRectangle(final double x,final double y,final double width,final double height, final double arcw, final double arch)
	{ _ShapeElement elt=new _ShapeElement(this)._STM(); elt.drawRoundRectangle(x,y,width,height,arcw,arch); return(elt); }
	   
	public _ShapeElement fillEllipse(final double x,final double y,final double width,final double height)
	{ _ShapeElement elt=new _ShapeElement(this)._STM(); elt.fillEllipse(x,y,width,height); return(elt); }
 
	public _ShapeElement fillRectangle(final double x,final double y,final double width,final double height)
	{ _ShapeElement elt=new _ShapeElement(this)._STM(); elt.fillRectangle(x,y,width,height); return(elt); }
	   
    public _ShapeElement fillRoundRectangle(final double x,final double y,final double width,final double height, final double arcw, final double arch)
    { _ShapeElement elt=new _ShapeElement(this)._STM(); elt.fillRoundRectangle(x,y,width,height,arcw,arch); return(elt); }
    ...
    class Drawing extends Canvas {
    	...
    	public void render() {
			...
			while(lnk!=null) {
				if(lnk instanceof _Drawing.Animable l) l.paint(g);
			    lnk=lnk.suc();
			}
			graphics.dispose();
			strategy.show();
...
//      currentFont=new Font(Font.SERIF,currentFont.getSize(),currentFont.getStyle());
	    ...
	    if(DEBUG) _RT.println("Init: Current Font="+currentFont);
		setFontSize(12);
		if(DEBUG) _RT.println("Init: Current Font="+currentFont);
	    frame.add(canvas);
	    frame.pack();
...
//      frame.moveToFront(); //VIRKER IKKE !!!
        frame.addWindowListener(new WindowListener() {
			public void windowOpened(WindowEvent e) { if(DEBUG) _RT.println("windowOpened.WindowsEvent: "+e); }
			public void windowClosing(WindowEvent e) { if(DEBUG) _RT.println("windowClosing.WindowsEvent: "+e);
				if(e.getID()==WindowEvent.WINDOW_CLOSING) {
					//printThreadList(true);
					System.exit(0);
				}
			}
			public void windowClosed(WindowEvent e) { if(DEBUG) _RT.println("windowClosed.WindowsEvent: "+e); }
			public void windowIconified(WindowEvent e) { if(DEBUG) _RT.println("windowIconified.WindowsEvent: "+e); }
			public void windowDeiconified(WindowEvent e) { if(DEBUG) _RT.println("windowDeiconified.WindowsEvent: "+e); }
			public void windowActivated(WindowEvent e) { if(DEBUG) _RT.println("windowActivated.WindowsEvent: "+e); }
			public void windowDeactivated(WindowEvent e) { if(DEBUG) _RT.println("windowClosing.WindowsEvent: "+e);
			}});
        canvas.setFocusable(true);
        canvas.addKeyListener(new KeyListener() {
        	public void keyTyped(KeyEvent e) {
				char c=e.getKeyChar();
				if(DEBUG) _RT.println("KEY "+c+" TYPED");
				if(c=='x') System.exit(0);
			}
			public void keyPressed(KeyEvent e) {
				if(DEBUG) _RT.println("KEY "+e.getKeyChar()+" PRESSED");
			}
			public void keyReleased(KeyEvent e) {
				if(DEBUG) _RT.println("KEY "+e.getKeyChar()+" RELEASE");
			}
        });
    }
