...
//		doPurgeAction();
	...
	public void locate(final int p) {
		if (p < 1 | p > _MAXLOC)
			throw new _SimulaRuntimeError("Locate: Parameter out of range");
		else
			try {
				randomAccessFile.seek((p - 1) * _RECORDSIZE);
			} catch (IOException e) {
				throw new _SimulaRuntimeError("Locate failed", e);
			}
		_LOC = p;
	}
