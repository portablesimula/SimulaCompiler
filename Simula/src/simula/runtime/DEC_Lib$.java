 ...
 *
 */
public class DEC_Lib$ extends CLASS$ {
	private static boolean VERBOSE=false;

	private static DEC_Lib$ DECLIB;

	// Constructor
    public DEC_Lib$(final RTObject$ staticLink) {
    	super(staticLink);
    	DECLIB=this;
    	BBLK(); // Iff no prefix
    }
     ...
     *
     * Abort will print the message on the error device and then
     * call terminate_program.
     */
	public static void abort(TXT$ mess) {
		RT.println(mess.edText());
		RTObject$.terminate_program();
	}
     ...
     *    end;
	 */
	public static boolean change(NAME$<TXT$> master, TXT$ oldtext, TXT$ newtext) {
		String oldt=oldtext.edText();
		String newt=newtext.edText();
		TXT$ Master=master.get();
		...
		if(diff > 0) {
			Master=new TXT$(newMaster);
			Master.POS=P+newt.length();
			master.put(Master);
		} else {
			TXT$ subt=TXT$.sub(Master,1,newMaster.length());
			DECLIB.ASGSTR$(subt,newMaster);
			Master.POS=P+newt.length();
	 ...
	 */
	public static TXT$ checkextension(TXT$ fileName, TXT$ defaultextension) {
		String f=fileName.edText();
		if(f.indexOf('.') >= 0) return(fileName);
		return(new TXT$(f + defaultextension.edText()));		
	}

	/**
	 *  <type> procedure check<type>(t); name t; text t;  
	 *         where <type> ::= real | int | frac
     *
     *    checkNumber analyses the text t from t.pos and on.
     ...
     *    
     *    NOTE: The current implementation will place t.pos after a complete real item, even if it overflows.
     *          Real underflow produces +0, overflow Infinity, both of which are treated as legal results 
	 */	
	public abstract class NumberChecker<T> {
		public abstract T check(TXT$ T) throws Exception;
	}

	public static int checkint(NAME$<TXT$> t) {
		NumberChecker<Number> intChecker = DECLIB.new NumberChecker<Number>() {
			public Integer check(TXT$ T){ return(TXT$.getint(T)); }};
		return(checkNumber(t,intChecker));
	}
	
	public static int checkfrac(NAME$<TXT$> t) {
		NumberChecker<Number> fracChecker = DECLIB.new NumberChecker<Number>() {
			public Integer check(TXT$ T){ return(TXT$.getfrac(T)); }};
		return(checkNumber(t,fracChecker));
	}
	
	public static int checkreal(NAME$<TXT$> t) {
		NumberChecker<Number> realChecker = DECLIB.new NumberChecker<Number>() {
			public Double check(TXT$ T){ return(TXT$.getreal(T)); }};
		return(checkNumber(t,realChecker));
	}
	
	private static int checkNumber(NAME$<TXT$> t,NumberChecker<Number> numberChecker) {
		if(VERBOSE) System.out.println("checkNumber: t="+t.get());
		TXT$ NT=t.get();
		if(VERBOSE) System.out.println("checkNumber: NT="+NT.edText()+", start="+NT.START+", length="+NT.LENGTH+", pos="+NT.POS);
		TXT$ U=TXT$.sub(NT,NT.POS+1,NT.LENGTH-NT.POS);
		if(VERBOSE) System.out.println("checkNumber: U="+U.edText()+", start="+U.START+", length="+U.LENGTH+", pos="+U.POS);
		if(U!=null) {
			try {
				if(VERBOSE) System.out.println("checkNumber: t="+U.edText()+", BEFORE,    t.pos="+U.POS);
				//TXT$.getreal(U);
				numberChecker.check(U);
				if(VERBOSE) System.out.println("checkNumber: t="+U.edText()+", return(1), t.pos="+U.POS);
				NT.POS=U.POS+NT.POS; t.put(NT);
				return(1);
			} catch (Exception e) { } 
		}
		String s=U.edText();
		...
		if(N==0) {
			...
			if(VERBOSE) System.out.println("checkNumber: t="+U.edText()+", return(0), t.pos="+U.POS);
			return(0);
		}
		if(VERBOSE) System.out.println("checkNumber: t="+U.edText()+", return(-1), t.pos="+U.POS+", N="+N);
		U.POS=U.LENGTH-N; t.put(U);
		return(-1);		
	}
	 ...
	 */
	public static TXT$ compress(TXT$ t,char c) {
        ...
        for(int p = 0; p < tinLen; p++) {
            ...
            if(tmpCh != c) { depchar(t,pOut,tmpCh); pOut++; }
        }
        if(pOut == 0) return(null);
        return(TXT$.sub(t,1,pOut-1));
	}
	 ...
	 * 
	 * Concatenate texts. Provided for compatibility only, use
     * the text concatenation operator & instead!
	 */
	public static TXT$ conc2(TXT$ t1,TXT$ t2) {
		return(new TXT$(t1.edText()+t2.edText()));		
	}
	public static TXT$ conc(TXT$ t1,TXT$ t2) {
		return(new TXT$(t1.edText()+t2.edText()));		
	}
	public static TXT$ conc3(TXT$ t1,TXT$ t2,TXT$ t3) {
		return(new TXT$(t1.edText()+t2.edText()+t3.edText()));		
	}
	public static TXT$ conc4(TXT$ t1,TXT$ t2,TXT$ t3,TXT$ t4) {
		return(new TXT$(t1.edText()+t2.edText()+t3.edText()+t4.edText()));		
	}
	public static TXT$ conc5(TXT$ t1,TXT$ t2,TXT$ t3,TXT$ t4,TXT$ t5) {
		return(new TXT$(t1.edText()+t2.edText()+t3.edText()+t4.edText()+t5.edText()));		
	}
     ...
     * SIMULA program execution, expressed in seconds. Provided for
     * compatibility, use cputime instead!
     *
     *    real procedure cptime;
     *    begin cptime:= cputime; end;
     *
	 * @return
	 */
	...
	public static int dayno() {
		return(TXT$.getint(TXT$.sub(datetime(),9,2)));		
	}
	 ...
	 */
	public static TXT$ daytime() {
		return(TXT$.sub(datetime(),12,8));		
	}
     ...
     * 
	 */
	public static void depchar(TXT$ t1,int p,char c) {
		if(p > 0 & p <= t1.LENGTH) {
			t1.POS=p-1; TXT$.putchar(t1, c);
		}
	}
     ...
     *
     * Enterdebug will activate SIMOB (if present). After SIMOB
     * session, the execution will either continue (maycontinue true)
     * or it will terminate (maycontinue false).
     *
...
//		if(SIMOB.isPresent) RT.NOT_IMPLEMENTED("DECLIB: enterdebug");
		if(!maycontinue) terminate_program();
	}
	...
	public static void exit(int code) {
		switch(code) {
			...
			case 1: terminate_program();
			case 2: enterdebug(true); terminate_program();
			default: RT.println("Parameter to exit out of range (0,2)");
					 terminate_program();
		}
	}
     ...
     *
	 */
	public static char fetchar(TXT$ t,int p) {
		if(p > 0 & p <= t.LENGTH) {
			t.POS=p-1; return(TXT$.getchar(t));
		}
		return(0);	
	}
     ...
     *
     *    character procedure findtrigger(master,triggers); name master; text master,triggers; begin
     *        character c;
     *        text t; t:- master;
     ...
     *
	 */
	public static char findtrigger(NAME$<TXT$> master,TXT$ triggers) {
		TXT$ M=master.get();
		String S=M.edText();
     ...
     *
	 */
	public static TXT$ from(TXT$ t,int p) {
		if(p<t.LENGTH) {
			if(p<0) return(t);
			else return(TXT$.sub(t, p, t.LENGTH-p+1));
		}
		return(null);
	}
     ...
     *
     *    text procedure front(t); text t;
     *    if t=/=notext then front:-t.sub(1,t.pos-1);
	 */
	public static TXT$ front(TXT$ t) {
		if(t!=null) return(TXT$.sub(t,1,t.POS));
		return(null);	
	}
	 ...
	 */
	public static boolean frontcompare(TXT$ string,TXT$ config) {
	 ...
	 */
	public static TXT$ frontstrip(TXT$ t) {
     ...
     *
     * Skips any blanks or tabs, starting at tt.pos. If tt.more
     * holds then, an item is identified according to the following rules:
     *
     *     a) If the first following character is a letter (a-z,A-Z), an
     *        identifier is found. The identifier consists of the initial   
     *         letter and any following letters and/or decimal digits.
     *
     *     b) If the first character is a digit, we have a numeric item,
     *         consisting of a string of digits with at most one decimal point
     *         "." included.
     *
     ...
     *
     * The value of getitem is a subtext reference to the item  within  tt,
     * or notext if no item can be found starting at tt.pos. tt.pos will be
     * placed after the item.
     *
     *
     *    text procedure getitem(tt); name tt; text tt;
     ...
     *
	 */
	enum State {NULL, NUMBER, IDENTIFIER, SYMBOL};
	public static TXT$ getitem(NAME$<TXT$> tt) {
		TXT$ TXT=tt.get();
		TXT$ RES=GETITEM(TXT);
		tt.put(TXT);
		return(RES); 
	...
	private static TXT$ GETITEM(TXT$ TXT) {
		String s=TXT.edText();
		String head=s.substring(0,TXT.POS);
		String tail=s.substring(TXT.POS);
		String rest=tail.stripLeading();
		int p=0; State state=State.NULL;
		LOOP:for(p=0;p<rest.length();p++) {
			char c=rest.charAt(p);
			switch(state) {
			    case NULL:
				    if(Character.isDigit(c)) state=State.NUMBER;
			        else if(Character.isLetter(c)) state=State.IDENTIFIER;
			        else state=State.SYMBOL;
				    break;
			    case SYMBOL: break LOOP;
			    case NUMBER: if(!(Character.isDigit(c)|c=='.'))    break LOOP; break;
			    case IDENTIFIER: if(!Character.isLetterOrDigit(c)) break LOOP; break;
 			}
		}
		int nSpace=tail.length()-rest.length();
		TXT.POS=head.length()+nSpace+p;
		return(new TXT$(rest.substring(0,p)));		
	}

	/**
     * integer procedure hash(t,n); text t; integer n;
     *
     * Procedure HASH returns a calculated hash value,
     * in the range (0:n-1), from a given text t.
     * The result is taken modulo n before return.
     *         
     ...
     *
     * The parameter is formally ref(file), but should refer an
     * infile, directfile, inbytefile or directbytefile (otherwise 
     * notext is returned). Same as getitem for files.
     * 
     * On infiles or directfiles, lastitem is called first. Notext is returned,
     * if (a) fileref is none, (b) fileref does not point to a legal
     * file object (see above), or (c) the remainder of the file is
     * spaces and/or TABs. Note, that since no copying is done, a
     * call on initem may destroy previously located items, if they
     * are not copied by the program, since inimage may be invoked.
     *
	 ...
	 * 
	 * Returns next input character from sysin (after last inimage)
	 * without waiting for break character. A subsequent inimage will
	 * begin reading after the last character which has been input
	 * with insinglechar.
	 * 
	...
	public static char insinglechar() {
		try { return(RT.console.read());
		} catch(Exception e) {
			throw new SimulaRuntimeError("Procedure insinglechar is undefined: re-run program with Runtime Option USE_CONSOLE=true");
		}
	}
     ...
     *
     * Returns uppercase character as lowercase equivalent,
     ...
     *
	 */
	public static TXT$ maketext(final char c,final int n) {
     ...
     *
     * Puts the (short) text newstring into the (longer) text oldstring.
     * Newstring is put at POS in oldstring, and POS of oldstring is
     *  moved after the end of the new substring.
     *  
     *  Returns false if there was not room for the new string,
     *          or if trying to modify a Constant text.
     *
     *  boolean procedure puttext(oldstring,newstring); name oldstring; text oldstring,newstring;
     *  begin text s;
     *      s:-oldstring;
     ...
     *  end;
	 */
	public static boolean puttext(NAME$<TXT$> oldstring,TXT$ newstring) {
	    TXT$ s=oldstring.get();
	    if(s.POS+newstring.LENGTH <= s.LENGTH & !TXT$.constant(s) ) {
	    	ASGTXT$(TXT$.sub(s,s.POS+1,newstring.LENGTH),newstring);
	        TXT$.setpos(s,s.POS+1+newstring.LENGTH);
	        oldstring.put(s);
	        return(true);
	    }
		return(false);		
     ...
     *
     *   text procedure rest(t); text t;
     *   rest :- t.sub(t.pos,t.length+1-t.pos);
     *
	 */
	public static TXT$ rest(TXT$ t) {
		int pos=t.POS+1;
		return(TXT$.sub(t,pos,t.LENGTH-pos+1));
	}
	
	/**
     * character procedure scanchar(t); name t; text t;
     *
     * Safe version of GETCHAR. Will not give RTS error,
     * just returns char(0) if not t.more.
     *
     *   character procedure scanchar(t); name t; text t;
     *   scanchar:= if t.more then t.getchar else char(0);
	 */
	public static char scanchar(NAME$<TXT$> t) {
		TXT$ T=t.get();
		if(!TXT$.more(T)) return(0);
		char res=TXT$.getchar(T); t.put(T);
		return(res);		
	}
	 ...
	 */
	public static int scanfrac(NAME$<TXT$> t) {
		TXT$ T=t.get();
		if(T!=null) {
			TXT$ TAIL=rest(T);
			try { int res=TXT$.getfrac(TAIL);
				  T.POS=T.POS+TAIL.POS; t.put(T);
				  return(res);
			} catch (Exception e) {	}
		}
		return(minint);		
	 ...
	 * 
	 */
	public static int scanint(NAME$<TXT$> t) {
		TXT$ T=t.get();
		if(T!=null) {
			TXT$ TAIL=rest(T);
			try { int res=TXT$.getint(TAIL);
				  T.POS=T.POS+TAIL.POS; t.put(T);
				  return(res);
			} catch (Exception e) {	}
		}
	 ...
	 * 
	 */
	public static double scanreal(NAME$<TXT$> t) {
		TXT$ T=t.get();
		if(T!=null) {
			TXT$ TAIL=rest(T);
			try { double res=TXT$.getreal(TAIL);
				  T.POS=T.POS+TAIL.POS; t.put(T);
				  return(res);
			} catch (Exception e) { }
		}
     ...
     *
     * Scans from tt.pos until the next occurrence of
     * the character c.  Places tt.pos after the character found.
     *         
     * The function value of scan is the text starting at the initial
     * tt.pos and not including the character c found.
     *         
     * If no character c is found, tt.pos will be = tt.length+1,
     * and scan denotes the rest of tt starting at the initial tt.pos.
     *
     ...
     *    end of scanto;
	 */
	public static TXT$ scanto(NAME$<TXT$> t,char c) {
		TXT$ TXT=t.get();
		String s=TXT.edText();
		
		String head=s.substring(0,TXT.POS);
		String tail=s.substring(TXT.POS);
		if(VERBOSE) System.out.println("scanto SPLIT: M=\""+s+"\", POS="+TXT.POS+", head=\""+head+"\", tail=\""+tail+'"');
		RT.ASSERT((head+tail).equals(s),head+tail+" NE "+s);
		
		int pos=TXT.POS+1;
		int index=tail.indexOf(c); // returns: the index of the first occurrence of the specified substring, or -1 if there is no such occurrence.
		if(VERBOSE) System.out.println("scanto: tail="+tail+", start="+TXT.START+", length="+TXT.LENGTH+", index="+index+", pos="+pos+", c="+c);
		if(index >= 0) { // Character found at index in tail
			TXT.POS=pos+index-1; t.put(TXT);
			if(VERBOSE) System.out.println("scanto(1): tail="+tail+", length="+TXT$.length(TXT)+", index="+index+", pos="+pos);
//			return(TXT$.sub(TXT,TXT.POS+1,tail.length()-index));
			return(TXT$.sub(TXT,pos,index));
		}
		else { // No character c is found
			TXT.POS=TXT.START+TXT.LENGTH; t.put(TXT);
			if(VERBOSE) System.out.println("scanto(2): tail="+tail+", length="+TXT$.length(TXT)+", index="+index+", pos="+pos);
			return(TXT$.sub(TXT,pos,TXT$.length(TXT)-pos+1));
		}
	}
     ...
     *	 */
	public static int search(TXT$ t1,TXT$ t2) {
     ...
     *
     * If, starting at tt.pos, a sequence of characters = c is found,
     ...
     *
	 */
	public static TXT$ skip(NAME$<TXT$> t,char c) {
		TXT$ TXT=t.get();
		String s=TXT.edText();
		int pos=TXT.POS;
		int lp=TXT.LENGTH-TXT.START;
		if(VERBOSE) System.out.println("skip: t="+TXT.edText()+", start="+TXT.START+", length="+TXT.LENGTH+", pos="+pos);
		TEXTOBJ obj=TXT.OBJ;
		while(pos < lp) {
			if(obj.MAIN[pos] != c) {
				TXT.POS=pos; TXT.LENGTH=TXT.LENGTH-pos; t.put(TXT);
				return(new TXT$(s.substring(pos)));
			}
			pos++;
		}
		TXT.POS=pos; t.put(TXT);
		if(VERBOSE) System.out.println("skip: t="+TXT.edText()+", return=null, pos="+pos);
		return null;		
	}
     ...
     *
     *   integer procedure startpos(t); text t;
     *   begin startpos:=t.start; end;
     *
	 */
	public static int startpos(TXT$ t) {
		return(t.START+1);		
	}
     ...
     *
     * Returns a reference to a text object of length 10 with contents
     *        "yyyy-mm-nn"
     * where yyyy is year, mm is month(in digits 01-12), nn is day (in digits 01-31).
     * This is the internationally standardized format for dates.
     *
     *   text procedure today; today:-datetime.sub(1,10);
	 */
	public static TXT$ today() {
     ...
     *
     * tsub is a safe variant of sub. Parameters which give
     ...
     *
	 */
	public static TXT$ tsub(TXT$ t,int pos,int length) {
		try { return(TXT$.sub(t,pos,length));
	 ...
	 *  **** upc(c);
     *
     *         Returns lowercase character as uppercase equivalent,
     *         other characters unchanged.
     *
     ...
     *
	 */
	public static boolean upcompare(TXT$ master,TXT$ test) {
		String mst=master.edText().toUpperCase().substring(TXT$.pos(master)-1);
		String tst=test.edText();
		if(VERBOSE) System.out.println("upcompare pos="+TXT$.pos(master));
		if(VERBOSE) System.out.println("upcompare mst="+mst);
		if(VERBOSE) System.out.println("upcompare tst="+tst);
     ...
     *
	 */
	public static TXT$ upto(TXT$ t,int p) {
		if(p<=0) return(null);
		if(p>t.LENGTH) return(t);
		return(TXT$.sub(t, 1, p-1));		
	}
