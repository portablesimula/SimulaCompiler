...
public final class SimulaScanner { 
    private static final int EOF_MARK=-1;    // used to denote end-of-input
    private LineNumberReader lineNumberReader; // actual source file reader;
    private Stack<Character> puchBackStack=new Stack<Character>();
	private String currentLine; // current Source Line
	private int currentPos;     // current pos in Source Line
	private int lastPos;        // last pos in Source Line
    ...
    */
	...
	private Token scanDirectiveLine() {
		//Util.BREAK("SimulaScanner.scanDirectiveLine: directive=\""+currentLine+'"');
		this.lastPos= -1;
		...
		if (tokenizer.hasMoreTokens()) {
			...
			else Util.warning("Unknown Compiler Directive: " + currentLine);
		}
	...
	private Token scanEndComment() {
		...
		int firstLine = Global.sourceLineNumber;
		int lastLine = firstLine;
		while (getNext() != EOF_MARK) { // if(!isWhiteSpace(current)) skipped.append((char)current);
			if (current == ';') {
				...
				String name = scanName();
				if (name.equalsIgnoreCase("END") || name.equalsIgnoreCase("ELSE") || name.equalsIgnoreCase("WHEN")
		...
		if (skipped.length() > 0)
			Util.error("END-Comment is not terminated");
		if (Option.TRACE_COMMENTS)
			Util.TRACE("ENDCOMMENT:\"" + skipped + '"');
		return (newItem(KeyWord.END));
	}
	...
	private void readNextLine() {
		try { currentLine=lineNumberReader.readLine();
		      if(currentLine!=null) currentLine=currentLine+"\n";
		      currentPos=0;
		      if(!EOF()) {
		    	  lastPos=currentLine.length();
		    	  Global.sourceLineNumber++;
		          if(Option.verbose) Util.LIST(currentLine);
		      }
		} catch(IOException e) { e.printStackTrace(); }
	}
    ...
    private int readNextCharacter() {
		...
		if(currentPos>=lastPos) readNextLine();
		if(EOF()) return(EOF_MARK);
		int c=currentLine.charAt(currentPos++);
	    if(c<32 && c!='\n') c=' '; // Whitespace
	    return(c);
	}
