...
{ boolean REAC;
  ...
  { Token activator=Parser.prevToken;
    ...
    if(Option.TRACE_PARSE) Util.TRACE("END NEW ActivationStatement: "+toString());
  }
  ...
  { String pri=""; if(prior) pri=" PRIOR";
	...
	switch(code) {
      case at:
      case delay:  return(activator+' '+code+' '+time+pri);
      case before:
      case after:  return(activator+' '+code+' '+object2);
      case direct:
