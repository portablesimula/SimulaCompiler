 ...
 */
public class OLD_BlockDeclaration extends DeclarationScope // Declaration implements Scope
...
//  }
  ...
  public OLD_BlockDeclaration(String identifier) { super(identifier); } 
  // Used by ClassDeclaration and ProcedureDeclaration
  public OLD_BlockDeclaration(String identifier,BlockKind blockKind)
...
//  public static BlockDeclaration createMaybeBlock(Variable blockPrefix,String blockIdentifier)
  public static OLD_BlockDeclaration createMaybeBlock(Variable blockPrefix)
  {	OLD_BlockDeclaration module=new OLD_BlockDeclaration(Global.sourceName);
 ...
 */
  ...
  {	BlockKind blockKind=(Option.standardClass)?BlockKind.Method:BlockKind.Procedure;
   	OLD_BlockDeclaration block=new OLD_BlockDeclaration(null,blockKind);
    block.type=type;  
 ...
 * </pre>
 */
  public static OLD_BlockDeclaration doParseProcedureDeclaration(Type type)
  {	BlockKind blockKind=(Option.standardClass)?BlockKind.Method:BlockKind.Procedure;
   	OLD_BlockDeclaration block=new OLD_BlockDeclaration(null,blockKind);
    block.type=type;  
    if(Option.TRACE_PARSE) Parser.TRACE("Parse ProcedureDeclaration, type="+type);
 ...
 * </pre>
 */
  public static OLD_BlockDeclaration doParseClassDeclaration(String prefix)
  { OLD_BlockDeclaration block=new OLD_BlockDeclaration(null,BlockKind.Class);
	block.prefix=prefix;
...
//	if(block.prefix==null) block.prefix=StandardClass.BASICIO.identifier;
   ...
   */
  public boolean isSubClassOf(OLD_BlockDeclaration other)
  { OLD_BlockDeclaration prefixClass=getPrefix();
...
//  }
  ...
  { Iterator<Parameter> prefixIterator;
    ...
    { OLD_BlockDeclaration prefix=getPrefix();
...
// 	else if(blockKind!=BlockDeclaration.Kind.External) externalIdent=edJavaClassName();
	...
	{ label.prefixLevel=prfx; label.index=labelIndex++;
      ...
      { DeclarationScope scope=label.myVirtual.declaredIn;
        ...
        else ((OLD_BlockDeclaration)label.declaredIn).virtualList.add(myVirtual=new VirtualSpecification(label)); 
      }
	}
    ...
    { DeclarationScope scope=myVirtual.declaredIn;
      ...
      else ((OLD_BlockDeclaration)declaredIn).virtualList.add(myVirtual=new VirtualSpecification(this)); 
    }
    Global.currentScope=declaredIn;
	if(blockKind!=BlockKind.CompoundStatement) currentBlockLevel--;
...
//	else line=line+" extends RTObject$";
    ...
    for(VirtualSpecification virtual:virtualList) virtual.doJavaCoding(indent+1);
...
//	  Util.code(indent,"     System.out.println(\""+getJavaIdentifier()+" Terminates Normally\");");
  ...
  {	Util.code(indent,"   // Normal Constructor");
	...
	Util.code(indent,"   "+line);
	if(prefix!=null) 
	{ OLD_BlockDeclaration prefixClass=this.getPrefix();
