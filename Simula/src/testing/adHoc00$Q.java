...
public class adHoc00$Q extends RTObject$ {
	// BlockKind=Procedure, BlockLevel=2, hasLocalClasses=false, System=false,
	// detachUsed=false
	...
	public adHoc00$Q setPar(Object param) { // Util.BREAK("CALL adHoc00$Q.setPar: param="+param+",
											// qual="+param.getClass().getSimpleName()+", npar="+$npar+",
											// staticLink="+SL);
		try {
			switch ($npar++) {
			case 0:
	...
	public adHoc00$Q STM() {
		BBLK();
		i = intValue(F.CPF().setPar(new $NAME<Integer>() {
			public Integer get() {
				return (4);
			}
