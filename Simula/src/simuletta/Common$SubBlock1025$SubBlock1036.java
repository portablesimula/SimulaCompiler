// JavaLine 1 <== SourceLine 1036
package simuletta;
// Simula-1.0 Compiled at Sun Apr 12 14:20:00 CEST 2020
import simula.runtime.*;
@SuppressWarnings("unchecked")
public final class Common$SubBlock1025$SubBlock1036 extends BASICIO$ {
    // SubBlock: Kind=SubBlock, BlockLevel=3, firstLine=1036, lastLine=1386, hasLocalClasses=false, System=false
    // Declare local labels
    // JavaLine 9 <== SourceLine 1085
    final LABQNT$ SETem=new LABQNT$(this,1,"SETem"); // Local Label #1=SETem
    // Declare locals as attributes
    // JavaLine 12 <== SourceLine 1036
    int i=0;
    char cs=0;
    TXT$ t=null;
    TXT$ listname=null;
    TXT$ feoptions=null;
    // Normal Constructor
    public Common$SubBlock1025$SubBlock1036(RTObject$ staticLink) {
        super(staticLink);
        BBLK();
        // Declaration Code
    }
        ...
        LOOP$:while(JTX$>=0) {
            try {
                ...
                if(VALUE$(TRF_NE(listname,null))) {
                    // JavaLine 82 <== SourceLine 1067
                    {
                        t=copy(copy(listname));
                        ;
                        t=lowcase(t);
                        ;
                        // JavaLine 88 <== SourceLine 1068
                        if(VALUE$(TXTREL$EQ(t,new TXT$("sysout")))) {
                            ((Common)(CUR$.SL$.SL$)).ListFile$1=sysout();
                        } else
                        // JavaLine 92 <== SourceLine 1069
                        {
                            // BEGIN INSPECTION 
                            ((Common)(CUR$.SL$.SL$)).inspect$1069$19$1=new PrintFile$(((BASICIO$)CTX$),listname).STM$();
                            if(((Common)(CUR$.SL$.SL$)).inspect$1069$19$1!=null) // INSPECT inspect$1069$19
                            // JavaLine 97 <== SourceLine 1070
                            {
                                ((Common)(CUR$.SL$.SL$)).listlength$1=getIntInfo(7);
                                ;
                                t=blanks(((Common)(CUR$.SL$.SL$)).listlength$1);
                                ;
                                // JavaLine 103 <== SourceLine 1071
                                if(VALUE$((!(((Common)(CUR$.SL$.SL$)).inspect$1069$19$1.open(t))))) {
                                    // JavaLine 105 <== SourceLine 1072
                                    {
                                        ((Common)(CUR$.SL$.SL$)).listlength$1=0;
                                        ;
                                        // JavaLine 109 <== SourceLine 1073
                                        new Common$error1(((Common)(CUR$.SL$.SL$)),(-(366)),listname);
                                    }
                                } else
                                ((Common)(CUR$.SL$.SL$)).ListFile$1=((PrintFile$)((Common)(CUR$.SL$.SL$)).inspect$1069$19$1);
                                ;
                            }
                        }
                        ;
                ...
                CUR$=THIS$;
                if(q.SL$!=CUR$) {
                    if(RT.Option.GOTO_TRACING) TRACE_GOTO("SubBlock1036:NON-LOCAL",q);
                    CUR$.STATE$=OperationalState.terminated;
                    if(RT.Option.GOTO_TRACING) TRACE_GOTO("SubBlock1036:RE-THROW",q);
                    throw(q);
                }
                if(RT.Option.GOTO_TRACING) TRACE_GOTO("SubBlock1036:LOCAL",q);
    ...
    public static PROGINFO$ INFO$=new PROGINFO$("COMMON.sim","SubBlock SubBlock1036",1,1036,9,1085,12,1036,30,1038,34,1040,37,1044,40,1045,43,1050,46,1051,49,1053,52,1054,55,1056,58,1057,61,1058,64,1059,67,1060,70,1061,75,1065,80,1066,82,1067,88,1068,92,1069,97,1070,103,1071,105,1072,109,1073,121,1076,124,1080,127,1081,130,1082,136,1083,145,1085,149,1086,154,1088,157,1089,160,1092,163,1093,166,1095,169,1096,172,1097,175,1098,178,1099,181,1100,184,1101,187,1102,190,1103,193,1104,196,1105,199,1106,202,1107,205,1108,208,1109,211,1111,214,1112,217,1113,220,1114,223,1116,226,1117,229,1118,232,1119,235,1121,238,1122,241,1123,244,1124,247,1126,250,1127,253,1128,256,1129,259,1131,262,1132,265,1133,268,1134,271,1136,274,1137,277,1138,280,1139,283,1141,286,1142,289,1143,292,1144,295,1146,298,1147,301,1148,304,1149,307,1151,310,1152,313,1153,316,1154,319,1156,322,1157,325,1158,328,1159,331,1161,334,1162,337,1163,340,1164,343,1166,346,1167,349,1168,352,1169,355,1171,358,1172,361,1173,364,1175,367,1177,370,1178,373,1179,376,1180,379,1182,382,1183,385,1184,388,1185,391,1187,394,1188,397,1189,400,1190,403,1192,406,1193,409,1194,412,1195,415,1197,418,1198,421,1199,424,1200,427,1202,430,1203,433,1204,436,1205,439,1207,442,1208,445,1209,448,1210,451,1212,454,1213,457,1214,460,1215,463,1217,466,1218,469,1219,472,1220,475,1222,478,1223,481,1224,484,1225,487,1227,490,1228,493,1229,496,1230,499,1232,502,1233,505,1234,508,1235,511,1237,514,1238,517,1239,520,1240,523,1242,526,1243,529,1244,532,1245,535,1247,538,1248,541,1249,544,1250,547,1252,550,1253,553,1255,556,1257,559,1258,562,1260,565,1261,568,1262,571,1263,574,1265,577,1266,580,1267,583,1268,586,1270,589,1271,592,1272,595,1273,598,1275,601,1276,604,1277,607,1278,610,1280,613,1281,616,1282,619,1283,622,1285,625,1286,628,1287,631,1293,634,1294,637,1295,640,1296,643,1298,646,1299,649,1300,652,1301,655,1303,658,1304,661,1305,664,1306,667,1308,670,1309,673,1310,676,1311,679,1313,682,1314,685,1315,688,1316,691,1318,694,1319,697,1320,700,1321,703,1323,706,1324,709,1325,712,1326,715,1328,718,1329,721,1330,724,1331,727,1333,730,1334,733,1335,736,1341,739,1342,742,1343,745,1344,748,1346,751,1347,754,1348,757,1349,760,1351,763,1352,766,1353,769,1354,772,1359,775,1360,778,1361,781,1362,784,1364,787,1365,790,1366,793,1367,796,1369,799,1370,802,1371,805,1372,808,1374,811,1375,814,1377,817,1382,820,1384,839,1386);
} // End of SubBlock
